...
- [Agent Plugin Detail](#agent-plugin-detail)
 ...
    - [Memory_메모리](#memory)
    - [Disk_디스크](#disk)
    - [Network_통신망_네트워크](#network)
    - [Monasca Agent_모나스카 에이전트](#monasca-agent)
    - [Limiting System Metrics_시스템 메트릭 ](#limiting-system-metrics)
  - [A10](#a10)
  - [Apache](#apache)
  - [Cacti](#cacti)
...
| Metric Name | Dimensions | Semantics |
| ----------- | ---------- | --------- |
| load.avg_1_min  |  | The normalized (by number of logical cores) average system load over a 1 minute period</br>(1분 단위로 발생한 정규화된 평균 시스템 부하 시간)
| load.avg_5_min  |  | The normalized (by number of logical cores) average system load over a 5 minute period</br>(5분 단위로 발생한 정규화된 평균 시스템 부하 시간)
| load.avg_15_min  |  | The normalized (by number of logical cores) average system load over a 15 minute period</br>(15분  발생한 정규화된 평균 시스템 부하 시간)
